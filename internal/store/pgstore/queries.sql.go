...
const removeReactionFromMessages = `-- name: RemoveReactionFromMessages :one
UPDATE messages
SET
  reaction_count = reaction_count - 1
WHERE
...
func (q *Queries) RemoveReactionFromMessages(ctx context.Context, id uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, removeReactionFromMessages, id)
	var reaction_count int64
	err := row.Scan(&reaction_count)
	return reaction_count, err
}
